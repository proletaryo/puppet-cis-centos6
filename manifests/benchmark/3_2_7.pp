...
class ciscentos6::benchmark::3_2_7 {
  if ! defined(Ciscentos6::Common::Add_file_line['3_2_7 net.ipv4.conf.all.rp_filter = 1']) {
    if $cis_benchmark_3_2_7 == 'failed' {   # remediate
      ciscentos6::common::add_file_line { '3_2_7 net.ipv4.conf.all.rp_filter = 1':
        filepath => '/etc/sysctl.conf',
        addline => 'net.ipv4.conf.all.rp_filter = 1',
        regex => '^net.ipv4.conf.all.rp_filter',
      } ->
      ciscentos6::common::add_file_line { '3_2_7 net.ipv4.conf.default.rp_filter = 1':
        filepath => '/etc/sysctl.conf'
        addline => 'net.ipv4.icmp_ignore_bogus_error_responses = 1',
        regex => '^net.ipv4.icmp_ignore_bogus_error_responses',
      } ->
      exec {'3_2_7 set the active kernel parameters':
